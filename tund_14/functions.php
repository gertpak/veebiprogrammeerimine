...
function readAllPublicPictureThumbsPage($page,$limit) {
	...
	$stmt->bind_result($idFromDB, $firstnameFromDB, $lastnameFromDB, $filenameFromDB, $alttextFromDB, $rateFromDB);
	$stmt->execute();
	while ($stmt->fetch()){
		...
		if($rateFromDB == NULL){
			$html .= '<p id="score' .$idFromDB .'">Hinne puudub' ."</p> \n";
		} else {
			$html .= '<p id="score' .$idFromDB .'">Hinne: ' .round($rateFromDB, 2) ."</p> \n";
		}
		$html .= "</div> \n";
	}
	if(empty($html)) {
...
//// Pekkis uudised
function saveNews($title, $text, $date) {
	$notice = "";
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
...
function showNews(){
	...
	$mysqli = new mysqli($GLOBALS["serverHost"], $GLOBALS["serverUsername"], $GLOBALS["serverPassword"], $GLOBALS["database"]);
	$stmt = $mysqli->prepare("SELECT userid, title, content, expire FROM vpnews WHERE expire>=?");
	echo $mysqli->error;
	$stmt->bind_param("s",$expiredateToday);
	$stmt->bind_result($userFromDB,$titleFromDB,$textFromDB,$expireFromDB);
	$stmt->execute();
	while ($stmt->fetch()){
		$html .= '<h3>'.$titleFromDB .'</h3>' ."\n";
		$html .= '<p>'.$textFromDB .'</p>' ."\n";
		$html .= "<p><i>Creator ID: ".$userFromDB ." Expires: " .$expireFromDB ."</i></p> \n";
	}
	if(empty($html)) {
		$html = "<p>Pole midagi kahjuks näidata.</p>";
	}
